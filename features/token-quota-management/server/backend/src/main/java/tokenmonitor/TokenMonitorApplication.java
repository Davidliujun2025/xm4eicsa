...
public final class TokenMonitorApplication {
    ...
    public static void main(String[] args) throws Exception {
        ...
        TokenUsageRepository repository = repository(env);
        ...
        TokenUsageService usage = new TokenUsageService(repository, zone, clock,
                dailyTokenLimit, highUsageThreshold, failureRateThreshold);
    ...
    private static TokenUsageRepository repository(Map<String, String> env) {
        String jdbcUrl = env.get("TOKEN_MONITOR_JDBC_URL");
        if (jdbcUrl == null || jdbcUrl.isBlank()) return new InMemoryTokenUsageRepository();
        return new JdbcTokenUsageRepository(new DriverManagerDataSource(
                jdbcUrl,
