...
import { CONVS, FILTERS } from "../mock/data";
import { Search, Plus } from "./icons";

export default function ConversationList({ platformName }: { platformName: string }) {
  const [filter, setFilter] = useState<string>("全部");
  const [active, setActive] = useState(0);
  const list = CONVS.filter((c) => filter === "全部" || c.tag === filter);
        ...
        {list.map((c, i) => (
          <button
            ...
            className={`group relative block w-full rounded-xl px-3 py-3 text-left transition-all duration-200  $ {
              active === i ? "bg-blue-50/80" : "hover:bg-slate-50"
            }`}
          >
            {active === i && <span className="absolute left-0 top-1/2 h-7 w-[3px] -translate-y-1/2 rounded-r bg-blue-600" />}
            <div className="flex items-start justify-between gap-2">
              <span className={`text-[13.5px] font-semibold  $ {active === i ? "text-slate-900" : "text-slate-700"}`}>{c.t}</span>
              <span className="shrink-0 text-[11.5px] text-slate-400">{c.time}</span>
            </div>
            <div className="mt-1.5 flex items-center gap-2 text-[11.5px]">
              <span className="font-medium text-blue-600">{platformName}</span>
